#!/usr/bin/perl
#
# File: column_select_substring.pl by Marc Perry
#
...
my $col = q{};
my $version = "0.0001\n";
my $usage = "Usage: column_select_substring.pl -l <list_file> -t <table_file> [-h] -c <column number in table_file>\n";

my $options = <<OPTIONS;
    Usage: column_select_substring.pl -l <list_file> -t <table_file> [-h] -c <column number in table_file>

    Options: 
        -l <list_file>     Specify single column text file containing the partial (incomplete) text patterns you wish to look for in the table_file
        -t <table_file>    Specify table file that you want to search 
        -c <column number> Specify the column number in the table that the grep will search when comparing the substrings from the list
...
open my ($MATCH), '>', 'column_select_substring_matched_rows_' . $timestamp . '.tsv' or die "Could not open column_select_substring_matched_rows.tsv for writing: $!";
open my ($NOT), '>', 'column_select_substring_unmatched_rows_' . $timestamp . '.tsv' or die "Could not open column_select_substring_unmatched_rows.tsv for writing: $!";
...
my $match = 0;
foreach my $row ( @rows ) {
    # In this new version of the script, instead of demanding that
    # the string pattern has to be a key in the %patterns_of_hash
    # This approach will select even partial matches
    # 
    # This will be slow (potentially) because for every row
    # in the table I am checking I have to iterate over every
    # element in the incoming list
